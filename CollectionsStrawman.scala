...
object CollectionStrawMan6 extends LowPriority {
  ...
  trait Builder[-A, +To] { self =>
    ...
    def mapResult[NewTo](f: To => NewTo): {f} Builder[A, NewTo] = new Builder[A, NewTo] {
  ...
  trait IterableLike[+A, +C[X] <: Iterable[X]]
     ...
     */
    protected[this] def fromIterableWithSameElemType(coll: {*} Iterable[A @uncheckedVariance]): {coll} C[A @uncheckedVariance] = fromIterable(coll)
  }
...
/* ---------- Iterators ---------------------------------------------------*/
  ...
  trait Iterator[+A] extends IterableOnce[A] { self =>
    ...
    def filter(p: A => Boolean): {p} Iterator[A] = new Iterator[A] {
    ...
    def map[B](f: A => B): {f} Iterator[B] = new Iterator[B] {
      def hasNext = self.hasNext
      def next() = f(self.next())
    }

    def flatMap[B](f: A => IterableOnce[B]): {f} Iterator[B] = new Iterator[B] {
