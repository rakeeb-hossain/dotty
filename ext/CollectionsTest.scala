...
object Test {
  ...
  def stringOps(xs: String) = {
    ...
    val ys8: String = xs8
    val xs9 = xs.map(x => {
      val z: Int = Char.char2int(x) + 1
      z
    })  // !!! need a language change to make this work without the : Char
    val ys9: Seq[Int] = xs9
  ...
  def main(args: Array[String]) = {
    ...
    val buf : ArrayBuffer[Int] = intsBuf.map( x => x + 2)
    seqOps(ints)
    seqOps(intsBuf)
    seqOps(intsListBuf)
    viewOps(intsView)
